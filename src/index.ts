...
const run = async () => {
  ...
  await userDAO.create(thirdUser);
  await userDAO.create(fourthUser);

  const updateObject: Partial<User> = { birthday: new Date(2009, 11 - 1, 27) };
  await userDAO.update(firstUserID, updateObject);
  const successfullyUpdatedUser = await userDAO.findOne(firstUserID);
  
  console.log('Data de nascimento atualizada: ');
  console.log(successfullyUpdatedUser);

  await userDAO.delete(firstUserID);
  const deletedUser = await userDAO.findOne(firstUserID);

  console.log('Usuário Matheus deletado: ');
